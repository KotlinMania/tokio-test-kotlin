# Claude Code Project Instructions — tokio-test-kotlin

## Project Overview

This is **tokio-test-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tokio-test`](https://crates.io/crates/tokio-test).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tokio-test/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tokio-test>
